...
labels = df["label"]
labels = np.array(labels[:1])  #taking 10 samples of labels from the dataset
inputs = df.drop(columns=["label"])  #dropping the label column
inputs = np.array(inputs[:1])  #taking 10 samples of inputs (pixels) from the dataset
iterations = 1  #number of epochs (forward propagation+backward propagation)
...
def training(weights , weights2 , biases1 , biases2):
    ...
    for i in range(iterations):
        for input, label in zip(inputs, labels):
            ...
            outputs = softmax(weights2.dot(layer1)+biases2)  #calculate the layer of output
            total_predictions += 1
            #mean_squared_error = (1/len(outputs)) * np.sum(np.square(np.subtract(expected_output, outputs))) #calculating mean squared error which is basically # subtracting the expected output from the
                                                                                                             # output squaring it and then summing it and diving it by length of row
            ...
            dw1 = ((weights2.T.dot(delta))*derivative(layer1)).dot(input.T)
            weights = weights - dw1
            db1 = ((weights2.T.dot(delta))*derivative(layer1))
            biases1 = biases1 - db1
...
'''changes to be made: change the loss function and recalculate derivatives'''